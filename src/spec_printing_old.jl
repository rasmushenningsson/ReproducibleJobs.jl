...
struct PrintNodeOld
	...
	children::Vector{PrintNodeOld}
end

create_print_node(pc::PrintContext, name::Symbol, h, item; children=PrintNodeOld[], item_color=:normal) =
	PrintNodeOld(pc, name, h, wrap_item(item), item_color, children)


AbstractTrees.children(x::PrintNodeOld) = x.children
...
function AbstractTrees.printnode(io::IO, x::PrintNodeOld; kwargs...)
	indent = x.pc.depth*3
	if x.name != Symbol("")
...
function print_spec_old(io::IO, spec::Spec; kwargs...)
