...
function run_deduplicator_tests()
	...
	@testset "Recursive transfer_ownership=$b" for b in (false, true)
		...
		@testset "$(typeof(x))" for x in [ [Reporter(b)],
		                                   ...
		                                   Set((Reporter(b),)),
		                                   Returns(Reporter(b)),
		                                   Base.Fix1(identity, Reporter(b)),
		                                   Base.Fix1(Reporter(b), 1),
		                                   Base.Fix1(Reporter(b), Reporter(b)),
		                                   Base.Fix2(identity, Reporter(b)),
		                                   Base.Fix2(Reporter(b), 1),
		                                   Base.Fix2(Reporter(b), Reporter(b)),
